...
> 一款面向团队协作的看板式任务管理平台，支持多工作区、拖拽排序、实时协同、AI 智能助手等功能。

## ✨ 核心功能

### 📋 看板管理
- **多工作区隔离** — 每个用户可创建/加入多个工作区，数据完全隔离
- **看板视图** — 支持创建多个看板，每个看板包含可自定义的列表（阶段）
- **卡片任务** — 在列表中创建卡片，支持标题、描述、截止日期
- **拖拽排序** — 基于 SortableJS 的流畅拖拽，支持卡片跨列表移动和列表排序
- **实时协同** — 基于 STOMP WebSocket 的实时数据同步，多人同时操作即时可见

### 👥 工作区协作
- **角色权限** — 三级角色体系：OWNER（所有者）> ADMIN（管理员）> MEMBER（成员）
- **邀请机制** — 生成邀请链接（24小时有效），通过邀请码加入工作区
- **成员管理** — 查看成员列表、移除成员、变更角色
- **在线状态** — 实时显示工作区内其他成员的在线状态

### 🤖 AI 智能助手
- **任务拆解** — 选择一张卡片，AI 自动将其拆解为 3-5 个子任务
- **周报生成** — AI 扫描工作区内所有卡片，自动生成结构化周报
...
### 📝 审计追踪
- **活动日志** — 记录卡片/列表/看板的创建、更新、删除、移动、评论等操作
- **变更对比** — 记录字段变更前后的值
...
```
┌─────────────────────────┐     ┌──────────────────────────────────┐
│   Frontend (Vue 3)      │────▶│   Backend (Spring Boot 3.4)      │
...
│   │   └── websocket.ts          # STOMP WebSocket 实时协作
│   ├── components/
...
### 后端 (`D:\IdeaFile\co-task-backend\co-tosk-backend`)
...
| **Workspace** | id, name, description, inviteCode |
| **WorkspaceMember** | workspace, user, role (OWNER/ADMIN/MEMBER) |
| **Board** | id, title, workspace |
| **TaskList** | id, title, position (Double), board |
| **Card** | id, title, description, position, dueDate, taskList |
| **Comment** | id, content, card, user |
| **ActivityLog** | id, actionType, entityType, oldValues, newValues |
| **AiTask** | id, taskType, prompt, result, status |
...
| STOMP 端点 | 说明 |
|------------|------|
| `/topic/workspace/{id}/card/moved` | 卡片移动广播 |
| `/topic/workspace/{id}/card/updated` | 卡片更新广播 |
| `/topic/workspace/{id}/list/updated` | 列表更新广播 |
| `/topic/workspace/{id}/presence/*` | 在线状态广播 |

### AI 助手
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/ai/decompose` | 任务拆解（异步） |
| POST | `/api/ai/summary/weekly-report` | 周报生成（异步） |
| GET | `/api/ai/risks/workspace/{id}` | 风险分析（同步） |
| GET | `/api/ai/task/{taskId}` | 查询异步任务结果 |
...
- **后端**: JDK 17, Maven 3.8+
- **数据库**: PostgreSQL 14+ (数据库名 `cotask_db`)
- **AI (可选)**: 豆包 API Key（环境变量 `DOUBAO_API_KEY`）

### 1. 启动后端

```bash
# 进入后端目录
cd D:\IdeaFile\co-task-backend\co-tosk-backend

# 确保 PostgreSQL 已启动，数据库 cotask_db 已创建
...
新用户注册时会自动创建一个默认工作区（包含一个演示看板和四个列表：待办事项、进行中、审核中、已完成）。
...
- **BCrypt 加密**: 用户密码使用 BCrypt 哈希存储
- **三级角色权限**: OWNER > ADMIN > MEMBER，每个 API 端点均有角色校验
- **WebSocket 鉴权**: 连接时通过 STOMP CONNECT 帧携带 Token，AuthChannelInterceptor 进行校验
- **无状态会话**: REST API 完全无状态，每次请求通过 Authorization Header 携带 JWT
...
- [ ] 子任务创建（AiAssistant 中已预留 UI，后端 API 待对接）
- [ ] 卡片标签/优先级系统
- [ ] 文件附件上传
- [ ] 看板筛选与搜索
- [ ] 邮件通知
